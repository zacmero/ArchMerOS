...
HYPR_THEME_LUA_FILE = REPO_ROOT / "config" / "hypr" / "theme.lua"
...
def write_files(name: str, palette: dict) -> None:
    ...
    HYPR_THEME_LUA_FILE.write_text(
        "\n".join(
            [
                "-- ArchMerOS theme values (generated by archmeros-theme.py)",
                "-- This file is overwritten by the theme engine; do not edit by hand.",
                "return {",
                f"    active_border   = {{ colors = {{ \"{rgba_hex(hex_to_rgb(palette['accent']), 1.0)}\", \"{rgba_hex(hex_to_rgb(palette['accent2']), 1.0)}\" }}, angle = 45 }},",
                f"    inactive_border = \"{rgba_hex(hex_to_rgb(palette['overlay0']), 0.68)}\",",
                f"    shadow          = \"{rgba_hex(css_color_to_rgb(palette['crust']), 0.62)}\",",
                "}",
                "",
            ]
        ),
